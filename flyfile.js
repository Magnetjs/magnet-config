...
export async function copyPackage(fly) {
  await fly.source('package.json').flatten().target('dist');
  await fly.source('LICENSE').flatten().target('dist');
  await fly.source('README.md').flatten().target('dist');
}
